...
class AvgAttentionReprojCell(nn.Module):
  def __init__(self, cfg={}, intrinsics=None, extrinsics=None):
    super(AvgAttentionReprojCell, self).__init__()
    ...
    attention_compression_rate = cfg['network']['prior']["attention_compression_rate"]
    attention_activation_func = blocks.SSMAActivationFunction.SIGMOID
    if 'tanh' in cfg['network']['prior']["attention_activation_func"].lower():
      attention_activation_func = blocks.SSMAActivationFunction.TANH
        
    # Create an SSMA attention module for all required layers
    ...
    for layer_name, seq_block in self.layers_dict.items() :
      if layer_name in self.fb_layers:
        ...
        self.attention_layers[layer_name] = blocks.SSMA_block(out_channels,
                                                              num_modes=2,
                                                              compression_rate=attention_compression_rate,
                                                              activation_func=attention_activation_func)
  ...
  def forward(self, x, priors={}):
    ...
    for layer_name, layer in self.layers_dict.items():
      ...
      if 'encoder' in layer_name:
        ...
        # additive feedback
        if layer_name in priors.keys():
          modes = (priors[layer_name], encode_blocks[-1])
          encode_blocks[-1] = self.attention_layers[layer_name](modes)
        # Replace old feedback with current
        if layer_name in self.fb_layers:
      ...
      if 'bottelneck' in layer_name:
        ...
        # additive feedback
        if layer_name in priors.keys():
          modes = (priors[layer_name], decode_block)
          decode_block = self.attention_layers[layer_name](modes)
        # Replace old feedback with current
        if layer_name in self.fb_layers:
...
class ReprojRNN(nn.Module):
  def __init__(self, cfg={}, intrinsics=None, extrinsics=None):
    ...
    if 'batch_norm_enabled' in cfg['network'].keys():
      blocks.set_apply_batch_norm_default(cfg['network']['batch_norm_enabled'])
    if 'dropout_probability' in cfg['network'].keys():
      blocks.set_dropout_probabaility_default(cfg['network']['dropout_probability'])
    ...
    self.segmentation_cell = AvgAttentionReprojCell(cfg, intrinsics, extrinsics)
